...
- [ ] Fix pnpm caching so the workflow never asks `actions/setup-node` to use
...
- [ ] Define the Trunk version once for the workflow. A restored or
  ...
  source.
- [ ] Add a least-privilege hosted correctness workflow for pull requests and
...
- [ ] Cache only reproducible development inputs and build outputs, with keys
  ...
  check.
- [ ] Add a hosted extended decoder job on `main`, on a schedule, or both. It
...
- [ ] Upload decoder logs and relevant failure evidence when an extended job
  fails. Successful jobs must not implicitly regenerate or commit golden
  fixtures or release evidence.
- [ ] Implement the documented test-only `quirc` secondary decoder for a
...
- [ ] Reconcile decoder documentation with the executable manifest: remove the
  ...
  evidence together.
- [ ] Remove transparent background from the public product and internal
...
- [ ] Use the owner-approved ordinary output: compact rounded modules, standard
  square finders, opaque white background, and the bundled logo by default.
  Keep the bundled logo as an explicit opt-in only within its existing ECC-H,
...
- [ ] Recalculate the selectable output matrix and resource/adverse baselines
...
- [ ] Add exact tests proving that removing transparency and changing defaults
...
- [ ] Update `docs/DEVELOPMENT_PLAN.md`, `docs/TESTING_STRATEGY.md`, release
...
- [ ] Run and record the relevant routine, independent-decode, adverse,
...
remembering to run local extended commands. The normal export should also make
the most robust scanner choice easy: opaque white, standard QR geometry, and no
occlusion unless the user deliberately enables the reviewed logo treatment.
