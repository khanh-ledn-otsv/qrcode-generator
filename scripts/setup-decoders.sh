...
case "${decoder_mode}" in
  ...
  zxing)
    ...
    ;;
  quirc)
    required_commands=(cc git make)
    ;;
  *)
    echo "Decoder setup mode must be all, zxing, or quirc; found '${decoder_mode}'." >&2
    exit 1
    ;;
esac
...
if [[ "${decoder_mode}" == "all" || "${decoder_mode}" == "zxing" ]]; then
...
if [[ "${decoder_mode}" == "all" || "${decoder_mode}" == "quirc" ]]; then
...
echo "Decoder setup complete (${decoder_mode})."
